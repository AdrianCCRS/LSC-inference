"""
LSC Real-Time Inference Script v2 — HG‑GCN style
==================================================
Interprete de Lengua de Senas Colombiana usando camara web + MediaPipe.
...
def normalize_landmarks(landmarks):
    ...
    Normaliza coordenadas de la mano:
      ...
      - Rota para alinear el vector muneca->MCP3 (landmark 9) al eje +X.
...
def build_node_features(coords_normalized, edges_undirected, base_connections=None):
    ...
    Identico a build_features() del notebook (version enriquecida con
    coordenadas esfericas y polares).
    """
    ...
    # ── Coordenadas esfericas / polares (HG-GCN enriquecimiento) ──────────
    x, y, z = c[:, 0], c[:, 1], c[:, 2]
    eps = np.finfo(np.float32).eps
    polar_r  = np.sqrt(np.maximum(x**2 + y**2, eps)).reshape(-1, 1)         # radio polar en XY
    azimuth  = np.arctan2(y, x).reshape(-1, 1)                              # angulo azimutal
    elevation = np.arctan2(z, np.sqrt(np.maximum(x**2 + y**2, eps))).reshape(-1, 1)  # elevacion desde XY
...
def build_tabular_features(coords_normalized):
    ...
    Construye el vector de features tabulares enriquecidas:
      ...
      distancias muneca-puntas + angulos articulares.
...
def _import_torch_geometric():
    try:
        ...
        return torch, Ft, Data, GCNConv, GATConv, GraphNorm, to_dense_batch
    except ImportError:
        return None
...
class HandGCNv2:
    ...
    @staticmethod
    def build(pyg_modules, in_channels, hidden_dims, num_layers, num_classes,
              num_nodes, dropout):
        torch, F, _, GCNConv, _, GraphNorm, to_dense_batch = pyg_modules
...
class HandGAT:
    ...
    @staticmethod
    def build(pyg_modules, in_channels, hidden_channels, num_classes,
              num_nodes, heads=4, dropout=0.3):
        torch, F, _, _, GATConv, GraphNorm, to_dense_batch = pyg_modules
        import torch.nn as nn
...
def load_model_package(model_key: str, artifacts_dir: Path):
    ...
    if pyg is None:
        raise ImportError("PyTorch / PyTorch Geometric no estan disponibles.")
...
def draw_overlay(frame_bgr: np.ndarray,
    ...
    # Titulo
    cv2.putText(out, "LSC Real-Time Interpreter (v2)", (14, 26),
...
def run_camera(pkg: dict,
    ...
    print(f"  Node features : 10‑dim (HG-GCN: coords + bones + polar)  |  v2")
    ...
    try:
        while True:
            ...
            cv2.imshow("LSC Interpreter v2", frame)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="LSC Real-Time Interpreter v2 -- inferencia desde camara.",
...
def list_models(artifacts_dir: Path):
    ...
    for key, fname in files.items():
        path   = artifacts_dir / fname
        status = "v encontrado" if path.exists() else "x NO encontrado"
